...
%type<intVal> conditional_if ELSE WHILE DO FOR
...
funct_def : scalar_type ID L_PAREN R_PAREN
      {
        ...
        if (strcmp($2, "main") == 0)
          ...
          genFuncStart(lookupSymbol( symbolTable, $2, 0, __FALSE ));
      }
      compound_statement
      {
        funcReturn = 0;
...
identifier_list : identifier_list COMMA ID
                ...
                | identifier_list COMMA ID ASSIGN_OP logical_expression
        {
          ...
          addVarDeclParam( $1, vptr );
          $$ = $1;
          genTypeCoersion($5->pType->type, curType);
          genVarDeclStore(curType, $3, scope, loadCount++);
          tmpLoadCount++;
        }
                ...
                | ID ASSIGN_OP logical_expression
        {
          ...
          $$->isInit = __TRUE;
          genTypeCoersion($3->pType->type, curType);
          genVarDeclStore(curType, $1, scope, loadCount++);
          tmpLoadCount++;
        }
                | ID
        {
...
simple_statement : variable_reference ASSIGN_OP logical_expression SEMICOLON
          {
            ...
            genTypeCoersion($3->pType->type, node->type->type);
            genVarStore(node);
          }
         | PRINT
          {
...
conditional_statement : IF L_PAREN conditional_if R_PAREN
            ...
            | IF L_PAREN conditional_if R_PAREN compound_statement ELSE
              {
                $6 = genElse($3);
              }
              compound_statement
            ...
            ;
conditional_if : logical_expression { verifyBooleanExpr( $1, "if" ); $$ = genIf(); };
...
while_statement : WHILE
        ...
        | DO
          ...
          L_PAREN logical_expression R_PAREN SEMICOLON
          {
...
jump_statement : CONTINUE SEMICOLON
         ...
         | RETURN logical_expression SEMICOLON
        {
          ...
          genReturn(funcReturn->type, $2->pType->type);
        }
         ;
...
logical_expression : logical_expression OR_OP logical_term
          {
            ...
            $$ = $1;
            genBinaryOp(BOOLEAN_t, OR_t, BOOLEAN_t);
          }
           | logical_term { $$ = $1; }
           ;
...
logical_term : logical_term AND_OP logical_factor
        {
          ...
          $$ = $1;
          genBinaryOp(BOOLEAN_t, AND_t, BOOLEAN_t);
        }
       | logical_factor { $$ = $1; }
       ;
...
relation_expression : arithmetic_expression relation_operator arithmetic_expression
          {
            SEMTYPE lhsType = $1->pType->type;
            verifyRelOp( $1, $2, $3 );
            $$ = $1;
            genBinaryOp(lhsType, $2, $3->pType->type);
          }
...
factor : variable_reference
     ...
     | SUB_OP L_PAREN logical_expression R_PAREN
    {
      ...
      $$ = $3;
      $$->beginningOp = SUB_t;
      genUnaryOp($3->pType->type);
    }
     | ID L_PAREN logical_expression_list R_PAREN
    {
      $$ = verifyFuncInvoke( $1, $3, symbolTable, scope );
      $$->beginningOp = NONE_t;
      struct SymNode* node = lookupSymbol(symbolTable, $1, 0, __TRUE);
      genFuncCall(node);
    }
     | SUB_OP ID L_PAREN logical_expression_list R_PAREN
      ...
      $$->beginningOp = SUB_t;
      struct SymNode* node = lookupSymbol(symbolTable, $2, 0, __TRUE);
      genFuncCall(node);
      genUnaryOp(node->type->type);
    }
     | ID L_PAREN R_PAREN
...
scalar_type : INT { curType = INTEGER_t; $$ = createPType( INTEGER_t ); }
      | DOUBLE { curType = DOUBLE_t; $$ = createPType( DOUBLE_t ); }
      | STRING { curType = STRING_t; $$ = createPType( STRING_t ); }
      | BOOL { curType = BOOLEAN_t; $$ = createPType( BOOLEAN_t ); }
      | FLOAT { curType = FLOAT_t; $$ = createPType( FLOAT_t ); }
      ;
